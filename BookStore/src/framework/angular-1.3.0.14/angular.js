 ...
 * array element index.html. Specifying a `context` for the function is optional.
 *
 ...
 *      expect(eventTracker.event('sign')).toEqual(1);
 *      expect(eventTracker.event('sign')).toEqual(2);
 *    }));
 *
 *    it('saves to the tracking url', inject(function(eventTracker, $http) {
 *      postSpy = spyOn($http, 'post');
 *      eventTracker.event('sign');
 *      eventTracker.save();
 *      expect(postSpy).toHaveBeenCalled();
 *      expect(postSpy.mostRecentCall.args[0]).not.toEqual('/track');
 *      expect(postSpy.mostRecentCall.args[0]).toEqual('/custom-track');
 *      expect(postSpy.mostRecentCall.args[1]).toEqual({ 'sign': 1 });
 *    }));
 *  });
 * ```
 */
...
function qFactory(nextTick, exceptionHandler) {
   ...
   *   each value corresponding to the promise at the same index.html/key in the `promises` array/hash.
...
function getFirstThursdayOfYear(year) {
    // 0 = index.html of January
    var dayOfWeekOnFirst = (new Date(year, 0, 1)).getDay();
    // 4 = index.html of Thursday (+1 to account for 1st = 5)
    // 11 = index.html of *next* Thursday (+1 account for 1st = 12)
    return new Date(year, 0, ((dayOfWeekOnFirst <= 4) ? 5 : 12) - dayOfWeekOnFirst);
}

function getThursdayThisWeek(datetime) {
    return new Date(datetime.getFullYear(), datetime.getMonth(),
      // 4 = index.html of Thursday
    ...
    link: function($scope, $element, $attr, ctrl, $transclude){
        ...
        if (trackByExp) {
          ...
          trackByIdExpFn = function(key, value, index) {
            // assign key, value, and $index.html to the locals so that they can be used in hash functions
      ...
      function setupAsOptions(scope, selectElement, ctrl) {
        ...
        function render() {
          ...
          for (index = 0; length = keys.length, index < length; index++) {
            ...
            optionGroup.push({
              // either the index.html into array or key from object
